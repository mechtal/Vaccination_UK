...
### Data types
```{r, echo=FALSE}
knitr::kable(sapply(py$dataset, typeof))
```

### Data description
```{r, echo=FALSE}
knitr::kable(describe(py$dataset))
```
...
ggplot(zeroes %>%
        group_by(z_third) %>%
        summarise(z_third_count=length(z_third)
)
, aes(x="", y=z_third_count, fill=z_third)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0)

ggplot(zeroes %>%
        group_by(z_first) %>%
        summarise(z_first_count=length(z_first)
)
, aes(x="", y=z_first_count, fill=z_first)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0)

ggplot(zeroes %>%
        group_by(z_second) %>%
        summarise(z_second_count=length(z_second)
)
, aes(x="", y=z_second_count, fill=z_second)) +
  geom_bar(stat="identity", width=1) +
  coord_polar("y", start=0)
```
...
```
```{r, echo=FALSE}
knitr::kable(filter(melted_cormat, Var1 %in% c("First", "Second"), Var2 %in% c("Month", "DayOfYear", "Quarter")))
```
