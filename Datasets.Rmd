...
```{r Datasets_Libraries, include=FALSE}
#---------------------------------------
...
#---------------------------------------
# Tick if the metrics belongs to the dataset
#---------------------------------------
# input: matrix where metrics are rows' indexes, vector_name that is the list of metrics and one of the columns
# output: matrix that has 1 on cross (metric, vector_name)

show_links <- function(matrix, vector_name) {
  for(metric in get(vector_name)) {
    matrix[metric, vector_name] = 1
  }
  return(matrix)
}
```
...
for(AreaType in keys(AreaTypesDict)){
  for(row in first) {
    if(M[row,AreaType] == 1){FirstDataset=append(FirstDataset,AreaType)}
  }
}

for(x in unique(FirstDataset)) {
  print(AreaTypesDict[[x]])
}
```