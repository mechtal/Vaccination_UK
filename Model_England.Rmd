...
```{r ModelEnglandArima, include=FALSE}
fit <- auto.arima(englandFit)
```

```{r ModelEnglandArimaSummary, echo=FALSE}
summary(fit)
```
ARIMA(5, 1, 3)
ARIMA(p,d,q)
...
```{r ModelEnglandTimeSeriesPacf, include=FALSE}
pacf(englandFit)
...
```{r ModelEnglandArimaForecastLower, echo=FALSE}
forecast(fit)$lower
```

```{r ModelEnglandArimaForecastUpper, echo=FALSE}
